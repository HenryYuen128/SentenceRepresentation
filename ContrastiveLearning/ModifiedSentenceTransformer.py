        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            for _ in trange(steps_per_epoch, desc="Iteration", smoothing=0.05, disable=not show_progress_bar):
                ...
                if evaluation_steps > 0 and training_steps % evaluation_steps == 0:
                    ...
                    if is_early_stopping and ((global_step - self.best_score_step)// evaluation_steps > patience):
                        print('Early stopping! Valid Score does not decrease in {} steps. The best score is {}'.format(patience*evaluation_steps, self.best_score))
            ...
            if is_stop_training:
                print('Stop training by early stopping.')
                break

        if evaluator is None and output_path is not None:   #No evaluator, but output path: save final model version
            print('Save model at {}'.format(output_path))
    ...
    def _eval_during_training(self, evaluator, output_path, save_best_model, epoch, steps, global_step, callback):
        ...
        if evaluator is not None:
            ...
            if score > self.best_score:
                print('Higher score {} at {} step. Last best score {} at {} step.'.format(score, global_step, self.best_score, self.best_score_step))
